...
public class TwelveProblems {
	...
	public static boolean endsWithUpperCaseLetter(String input) {
		boolean out;
		if(input.length()>0) {
			out=Character.isUpperCase(input.charAt(input.length()-1));
		}
		else {out=false;}
		return out;
	}
	...
	public static char mostCommonCharacter(String input) {
		...
		for (int i=0;i<input.length();i++) {
			...
			for (int j=0;j<input.length();j++) {
				if(input.charAt(j)==tempchar)tempcount++;
			}
			if(tempcount>comcount) {
				comcount=tempcount;
				comchar=tempchar;
			}
		}
	...
	public static int[] maxArray(int[] one, int[] two) {
		int[] nums=new int[one.length];
		for(int i=0;i<one.length;i++) {
			if(one[i]>two[i])nums[i]=one[i];
			else nums[i]=two[i];
		}
		return nums;
	}
	...
	public static int timesOccur(int[] shorter, int[] longer) {
		...
		for(int i=0;i<=longer.length-shorter.length;i++) {
			boolean match=false;
			if(shorter[0]==longer[i]) {
				match=true;
				for(int j=0;j<shorter.length;j++) {
					if(shorter[j]!=longer[i+j])match=false;
				}
				if(match)occ++;
			}
