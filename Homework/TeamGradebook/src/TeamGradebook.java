...
public class TeamGradebook {
	...
	private String handleAddStudent(String studentName) {
		...
		// then return "ok"
		Student next = new Student(studentName);
		students.add(next);
		return "ok";
	}
	...
	private String handleAddTeam(String teamName, ArrayList<String> memberNames) {
		...
		for(String name:memberNames) {
			...
			members.add(getStudentByName(name));
		}
		Team next = new Team(teamName, members);
		teams.add(next);
		return "ok";
	}
	...
	private String handleAddGrade(String teamName, double grade) {
		ArrayList<Student> t = getTeamByName(teamName).getMembers();
		for(Student stud:t) {
			stud.addGrade(grade);
		}
	...
	/**
	 * Finds the team object with the given name in the teams list
	 * 
	 * @param teamName
	 * @return team with the given name or null
	 */
	...
	private String handleGetAverage(String studentName) {
		return Integer.toString(getStudentByName(studentName).averageGrade());
	}
	...
	private String handleGetBestTeam() {
		...
		for(Team t:teams) {
			if(t.averageGrade()>bestAverage) {
				bestAverage=t.averageGrade();
				bestTeam=t.getName();
			}
		}
		return bestTeam;
